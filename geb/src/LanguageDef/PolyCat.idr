...
-- or as the functor minus its value at zero) of the functor which generates
-- unrefined finite types.
public export
data FinTVarF : Type -> Type where
  FTVCoproduct : carrier -> carrier -> FinTVarF carrier
  FTVProduct : carrier -> carrier -> FinTVarF carrier

-- The constant component of the functor which generates finite types.
public export
data FinTConst : Type where
  FTVInitial : FinTConst
  FTVTerminal : FinTConst

-- The complete functor which generates unrefined finite types.
public export
data FinTF : Type -> Type where
  FTFConst : FinTConst -> FinTF carrier
  FTFVar : FinTVarF carrier -> FinTF carrier

-- The type of types generated by any of the first `N` iterations of
-- `FinTF` on top of `Void`.
public export
FinTFDepth : Nat -> Type
FinTFDepth Z = Void
...
minDepth {n=(S n)} (FTFConst _) = S Z
minDepth {n=(S n)} (FTFVar (FTVCoproduct x y)) = smax (minDepth x) (minDepth y)
minDepth {n=(S n)} (FTFVar (FTVProduct x y)) = smax (minDepth x) (minDepth y)

public export
minDepthCorrect : {n : Nat} ->
